...
* Fixed: applying or undoing a link deleted every backslash in the post. WordPress strips one level of slashes from whatever it is handed before it saves, so content has to be escaped on the way in and this plugin was not escaping it. A post holding a Windows path, a regular expression, a LaTeX formula or a code sample lost a backslash from each one every time a link was applied to that post or taken off it. Both writes are now escaped, so what goes in is what comes out. Existing posts keep whatever was already lost — this stops it happening again, it cannot recover what earlier writes removed.
* Fixed: "Reset all data" stranded every link the plugin had inserted. Reset emptied the inserted-links log along with the scan data, and that log is the only record of which links the plugin put into your content. Once it was gone the links stayed in the posts but nothing could reach them: no per-link Undo, no "Remove all inserted links", and an uninstall that could no longer find them to take out. They became permanent, untracked edits. The warning on the screen did mention it, which made it a known risk rather than a hidden one, but a warning is not a safeguard.
* The undo record now survives a reset — both the log itself and the applied rows that carry the Undo button, since that button is rendered from the suggestion rather than from the log, and keeping one without the other would preserve the record with no way left to reach it. Everything a rescan genuinely rebuilds still clears: the index, the link graph, keywords, and every suggestion you have not applied. Entries for links already taken out clear too, since there is nothing left to undo.
* The Reset card now says what it keeps rather than what it destroys, and counts the links on your site that a reset will leave untouched. This is the same correction made in 0.13.0, when reset was emptying the API key pool: a reset is for clearing what a rescan rebuilds, and nothing about scanning again is helped by forgetting what was written into someone's posts.
* Fixed: Link Health reported working pages as broken links. A link was called broken whenever it did not resolve to a post in the plugin's own index, which is a different question with a much worse answer — every link to a category, tag, author, date or shop archive was reported, because none of those is a post, and so was every link to a healthy page in a post type you had chosen not to crawl. The few genuinely broken links were buried among pages that were never broken, and anyone acting on the report would have gone and "fixed" working pages.
* The plugin now asks WordPress what an unresolved URL actually is: the front page, a search, a feed, a post-type archive, a category or tag archive for any taxonomy, an author page, a date archive, or page two of any of those. Each answer has to be earned — a category counts only if that category exists and its own address is the link in question — so a link to a category that does not exist is still reported, which is what the report is for. Links that do resolve are now checked against the post itself rather than against the index, so a published page outside your crawl scope is no longer called broken for being outside it.
* Links written with an anchor, like /about/#team, resolve to the page rather than to nothing. They were being reported as broken against pages that were plainly there.
* Fixed: pages in your navigation menu were reported as orphans. The orphan audit only looked at links inside article bodies, so a page in the main menu — reachable in one click from every page on the site — was listed as having nothing linking to it. Menu links now count. They count for the orphan list only: dead ends and link density are about what an author wrote into the page, and the PageRank score stays content-only, because a menu links from everywhere to the same few pages and would flatten the very structure that score measures.
* Both lists now say what they cover. Links from widgets, footers and shortcodes are still not read — they come in too many shapes to read reliably — so a page listed as an orphan may still be reachable from one, and the screen says so. It also says that nothing is fetched over HTTP: this reports what WordPress can route, so a page that exists but whose server returns an error cannot be detected here.
